#!/system/bin/sh
# install_direct.sh
# Run AFTER you have root in adb shell:
#   adb shell
#   su                    <- tap ALLOW on phone
#   sh /sdcard/install_direct.sh

MOD_ID="force_sim_network_mode"
MOD_DIR="/data/adb/modules/$MOD_ID"
...
# Verify we are root
CURRENT_UID=$(id | cut -d'=' -f2 | cut -d'(' -f1)
if [ "$CURRENT_UID" != "0" ]; then
    echo "[ERROR] Must run as root! Run: su, then try again."
    exit 1
fi
...
RETRY=0
until settings get global preferred_network_mode1 > /dev/null 2>&1; do
    sleep 5; RETRY=$((RETRY+1))
    [ $RETRY -ge 12 ] && { log_msg "ERROR: timeout"; exit 1; }
done
...
# Remove any disable/remove flags
rm -f "$MOD_DIR/disable" "$MOD_DIR/remove"
